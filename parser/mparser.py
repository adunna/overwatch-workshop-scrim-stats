...
class MatrixParser:
    ...
    def readLog(self, filename):
        ...
        with open(filename, "r", encoding="utf-8") as f:
            for line in f:
                if line[0] == "[":
                    line = line.replace("McCree", "Cassidy")
                    if "," in line:
        ...
        for lineNumber, line in enumerate(PARSED_LINES[1:]):
            ...
            if len(line) == 0:  # empty line
            ...
            elif (game.language == LANG_EN and line[0] == game.map) or (
                game.language == LANG_KR
                and line[0] in KR_REMAP_MAPS
                and KR_REMAP_MAPS[line[0]] == game.map
            ):  # new part of map / next side
                for team in game.player_tracking[-1]:
                    for player in team:
                        game.section_lengths[-1] = min(
                            game.section_lengths[-1], len(team[player].stats["heroes"])
                        )
                ...
                if len(line) == 3:
                    if line[2] == "Death":  # hero death
                        if (
                            PARSED_LINES[lineNumber - 1][2] == "FinalBlow"
                            and abs(float(PARSED_LINES[lineNumber - 1][0]) - timestamp)
                            <= 0.05
                        ) or (
                            PARSED_LINES[lineNumber + 1][2] == "FinalBlow"
                            and abs(float(PARSED_LINES[lineNumber + 1][0]) - timestamp)
                        ...
                        game.kill_tracking[-1].append(
                            (runningTS, line[2], line[2], "Suicide")
                        )
                        ...
                        if line[2] not in game.overall_deaths[-1]:
                            ...
                            len(game.overall_deaths[-1][line[2]]) > 0
                            and runningTS - game.overall_deaths[-1][line[2]][-1] <= 10
                        ):  # make sure player died recently
                            # check that player was not rezzed recently
                            not_rez = False
                            for rez_point in game.rez_tracking[-1]:
                                if (
                                    rez_point[1] == line[2]
                                    and runningTS - rez_point[0] <= 10
                                ):
                                    not_rez = True
                            if not not_rez:
                        ...
                        if (
                            line[2] not in game.dupe_tracking[-1]
                        ):  # dupe did not stop before match end
                            game.dupe_tracking[-2][line[2]][-1][
                                1
                            ] = game.section_lengths[-2]
                        ...
                        if game.map_type == "Control":
                            ...
                            game.map_tracking[-1][-1].attacker = (
                                0 if line[1] == "True" else 1
                            )
                            game.map_tracking[-1][-1].progress = float(line[2])
                        if (
                            game.map_type == "Hybrid" or game.map_type == "Assault"
                        ) and len(game.map_tracking[-1]) > 2:
                            game.map_tracking[-1][-1].pointCaptured = game.map_tracking[
                                -1
                            ][-2].pointCaptured
                            if (
                                game.map_tracking[-1][-1].progress == 0
                                and game.map_tracking[-1][-2].progress >= 80
                            ):
                                game.map_tracking[-1][-1].pointCaptured = True

                    if TRACK_KILLER is not None and TRACK_VICTIM is not None:
                        game.kill_tracking[-1].append(
                            (runningTS, TRACK_KILLER, TRACK_VICTIM, "Unknown")
                        )
                        TRACK_KILLER = None
                        TRACK_VICTIM = None
                    ...
                    if line[1] == "DuplicatingStart":
                        ...
                        if game.language == LANG_EN:
                            game.dupe_tracking[-1][line[2]].append(
                                [runningTS, 0, line[3]]
                            )
                        elif game.language == LANG_KR:
                            game.dupe_tracking[-1][line[2]].append(
                                [runningTS, 0, KR_REMAP_HEROES[line[3]]]
                            )

                elif len(line) == 5:
                    if line[1] == "FinalBlow":
                        game.kill_tracking[-1].append(
                            (runningTS, line[2], line[3], line[4])
                        )  # later will add what died to, which is line[4]
                    ...
                    if len(line) >= 24:
                        ...
                        playerTeam = "Team 1"
                    if (
                        playerTeam not in game.team_names
                    ):  # outdated but still has team names
                    ...
                    if playerName not in game.player_tracking[-1][playerTeam]:
                        game.player_tracking[-1][playerTeam][
                            playerName
                        ] = MatrixPlayer()
                        game.player_tracking[-1][playerTeam][
                            playerName
                    ...
                    try:
                        game.player_tracking[-1][playerTeam][playerName].stats[
                            "position"
                        ].append(
                            (float(line[20][1:]), float(line[21]), float(line[22][:-1]))
                        )
                        if game.language == LANG_EN:
                            if line[2] in HERO_REMAPS:
                                game.player_tracking[-1][playerTeam][playerName].stats[
                                    "heroes"
                                ].append(HERO_REMAPS[line[2]])
                            elif "Torb" in line[2]:
                                game.player_tracking[-1][playerTeam][playerName].stats[
                                    "heroes"
                                ].append("Torbjorn")
                            ...
                            game.player_tracking[-1][playerTeam][playerName].stats[
                                "heroes"
                            ].append(KR_REMAP_HEROES[line[2]])

                        # Patch numbers that are asterisks
                        for stat in STAT_NUMBER_MAPS:
                            if (
                                STAT_NUMBER_MAPS[stat] < len(line)
                                and "*" in line[STAT_NUMBER_MAPS[stat]]
                                and len(
                                    game.player_tracking[-1][playerTeam][
                                        playerName
                                    ].stats["hero_damage_dealt"]
                                )
                                != 0
                            ):
                                line[STAT_NUMBER_MAPS[stat]] = str(
                                    game.player_tracking[-1][playerTeam][
                                        playerName
                                    ].stats[stat][-1]
                                )
                        ...
                        game.player_tracking[-1][playerTeam][playerName].stats[
                            "healing_received"
                        ].append(float(line[17]))
                        game.player_tracking[-1][playerTeam][playerName].stats[
                            "ultimate_charge"
                        ].append(float(line[18]))
                        game.player_tracking[-1][playerTeam][playerName].stats[
                            "player_closest_reticle"
                        ].append(line[19])
                        game.player_tracking[-1][playerTeam][playerName].stats[
                            "cooldown1"
                        ].append(float(line[24]))
                        game.player_tracking[-1][playerTeam][playerName].stats[
                            "cooldown2"
                        ].append(float(line[25]))
                        if len(line) > 26:
                            game.player_tracking[-1][playerTeam][playerName].stats[
                                "max_health"
                            ].append(float(line[26]))
                        else:
                            game.player_tracking[-1][playerTeam][playerName].stats[
                                "max_health"
                            ].append(0)
                        ...
                        if game.version >= Version.V0_6:
                            ...
                            game.player_tracking[-1][playerTeam][playerName].stats[
                                "charge_ability1"
                            ].append(float(line[29]))
                            game.player_tracking[-1][playerTeam][playerName].stats[
                                "charge_ability2"
                            ].append(float(line[30]))
                            game.player_tracking[-1][playerTeam][playerName].stats[
                                "resource_primary"
                            ].append(float(line[31]))
                            game.player_tracking[-1][playerTeam][playerName].stats[
                                "resource_secondary"
                            ].append(float(line[32]))
                            game.player_tracking[-1][playerTeam][playerName].stats[
                                "resource_ability1"
                            ].append(float(line[33]))
                            game.player_tracking[-1][playerTeam][playerName].stats[
                                "resource_ability2"
                            ].append(float(line[34]))
                            game.player_tracking[-1][playerTeam][playerName].stats[
                                "weapon_accuracy"
                            ].append(float(line[35]))

                        if (
                            game.player_tracking[0][playerTeam][playerName].dc_stats[
                                "heroes"
                            ]
                            is not None
                        ):
                            for stat in MatrixPlayer.STAT_TYPES["number_update"]:
                                if (
                                    ...
                                    game.player_tracking[-1][playerTeam][
                                        playerName
                                    ].stats[stat][-1] += game.player_tracking[0][
                                        playerTeam
                                    ][
                                        playerName
                                    ].dc_stats[
                        ...
                        if (
                            runningTS > prevRunningTS
                        ):  # check if missing any stats for a player, and if so, pad with previous stats
                            for team in game.player_tracking[-1]:
                                for player in team:
                                    timeDiff = (
                                        runningTS
                                        - len(team[player].stats["heroes"])
                                        - 1
                                    )
                                    while timeDiff > 0:
                                        for stat in team[player].stats:
                                            if len(team[player].stats[stat]) > 0:
                                                team[player].stats[stat].append(
                                                    team[player].stats[stat][-1]
                                                )
                                        timeDiff -= 1
                            prevRunningTS = runningTS
                    except Exception as e:
                        print(e)
                        for stat in game.player_tracking[0][playerTeam][
                            playerName
                        ].dc_stats:
                            if (
                                len(
                                ...
                                game.player_tracking[0][playerTeam][
                                    playerName
                                ].dc_stats[stat] = game.player_tracking[-1][playerTeam][
                                    playerName
                                ].stats[
                                    stat
                                ][
                        ...
                        if (
                            ...
                            == "D.Va"
                            and game.player_tracking[-1][playerTeam][
                                playerName
                            ].dc_stats["max_health"]
                            <= 200
                            and len(
                                [
                                    x[0]
                                    for x in game.kill_tracking[-1]
                                    if x[2] == playerName and runningTS - x[0] > 10
                                ]
                            )
                            == 0
                        ):  # baby dva
                            game.player_tracking[0][playerTeam][playerName].dc_stats[
                                "damage_taken"
                            ] -= 600  # adjust for mech damage will take

        for section_num, section in enumerate(game.player_tracking):
            for team in section:
                for player in team:
                    game.section_lengths[section_num] = min(
                        game.section_lengths[section_num],
                        len(team[player].stats["heroes"]),
                    )
                ...
                for ts in range(0, game.section_lengths[section]):
                    for team in range(0, len(game.player_tracking[section])):
                        for player in game.player_tracking[section][team]:
                            if ts < len(
                                game.player_tracking[section][team][player].stats[
                                    "hero_damage_dealt"
                                ]
                            ):
                                ...
                                o.write(
                                    ...
                                    % (
                                        ...
                                        game.team_names[0]
                                        if team == 0
                                        else game.team_names[1],
                                        player,
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["heroes"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["hero_damage_dealt"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["barrier_damage_dealt"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["damage_blocked"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["damage_taken"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["deaths"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["eliminations"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["final_blows"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["environmental_deaths"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["environmental_kills"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["healing_dealt"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["objective_kills"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["solo_kills"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["ultimates_earned"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["ultimates_used"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["healing_received"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["ultimate_charge"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["player_closest_reticle"][ts],
                                        game.player_tracking[section][team][
                                            player
                                        ].stats["cooldown1"][ts],
                                        game.player_tracking[section][team][
                                ...
                                if game.version >= Version.V0_6:
                                    o.write(
                                        ...
                                        % (
                                            game.player_tracking[section][team][
                                                player
                                            ].stats["offensive_assists"][ts],
                                            game.player_tracking[section][team][
                                                player
                                            ].stats["defensive_assists"][ts],
                                            game.player_tracking[section][team][
                                                player
                                            ].stats["charge_ability1"][ts],
                                            game.player_tracking[section][team][
                                                player
                                            ].stats["charge_ability2"][ts],
                                            game.player_tracking[section][team][
                                                player
                                            ].stats["resource_primary"][ts],
                                            game.player_tracking[section][team][
                                                player
                                            ].stats["resource_secondary"][ts],
                                            game.player_tracking[section][team][
                                                player
                                            ].stats["resource_ability1"][ts],
                                            game.player_tracking[section][team][
                                                player
                                            ].stats["resource_ability2"][ts],
                                            game.player_tracking[section][team][
                                                player
                                            ].stats["weapon_accuracy"][ts],
                                        )
                                    )
