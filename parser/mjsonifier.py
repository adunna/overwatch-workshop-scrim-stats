...
class MatrixJSON:
    ...
    def DumpJSON(self):
        ...
        for section in self.Analyzer.game.kill_tracking:
            for kill in section:
                match_events['kills'].append({'id': match_event_id, 'start': last_time_end + kill[0], 'type': 'point', 'group': kill[1], 'content': '', 'title': "Killed " + kill[2]})
                match_event_id += 1
                match_events['kills'].append({'id': match_event_id, 'start': last_time_end + kill[0], 'type': 'point', 'group': kill[2], 'content': '', 'title': "Died to " + kill[1]})
                match_event_id += 1
